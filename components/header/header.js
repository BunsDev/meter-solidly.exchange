      ...
      <div className={classes.chainInvalidError}>
        <div className={classes.ErrorContent}>
          ...
          <Typography className={classes.ErrorTxt}>
            The chain you're connected to isn't supported. Please check that your wallet is connected to Meter Testnet.
          </Typography>
          <Button className={classes.switchNetworkBtn} variant="contained" onClick={()=>switchChain()} >Switch to { process.env.NEXT_PUBLIC_CHAINID == '83' ? 'Meter Testnet' : 'Meter Mainnet' }</Button>
        </div>
      </div>
    ) : null}
    </div>
